...
- A new or changed query ships its index in `firestore.indexes.json` in the same phase that deploys the Function using it, or the query fails in production only. Not every index is a composite one: a **single-field** collection-group query needs a `fieldOverrides` entry with `COLLECTION_GROUP` scope, because Firestore only creates single-field indexes at `COLLECTION` scope automatically. Naming a field there *replaces* its automatic indexing, so list the `COLLECTION` orders you still want alongside the new one. `scripts/index-contract.mjs` checks this against the actual queries — if it complains, it has read your query and the answer is in its message.

...
| TD-C1 | call 啟動延遲只有靜態證據：`src/calls/call-timing.ts` 的分段量測已就緒，但尚未在 production 取得任何一次真實數據 | 目前無法分辨瓶頸是 Cloud Function 冷啟動、App Check、token 發放、LiveKit 連線或媒體授權；後續最佳化只能靠猜 | 中 | 需要真實 provider 與已部署的 RTC callables | 立即 | 在 production 以 `localStorage['chat-lite:call-timing']='1'` 取得冷啟動與熱啟動各一組分段數字，記入 `docs/RTC.md`；若瓶頸在 client 之外，標記 `MEASURED-BUT-NOT-CLIENT-FIXABLE` 並停止 client 端最佳化 |
